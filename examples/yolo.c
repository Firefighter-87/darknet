...
void train_yolo4(char *cfgfile, char *weightfile)
{
    char* train_images = "train.txt";
    char* backup_directory = "backup";
    srand(time(0));
...
void print_yolo4_detections(FILE **fps, char *id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    int i, j;
    for(i = 0; i < total; ++i){
...
void validate_yolo4(char *cfgfile, char *weightfile)
{
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            print_yolo4_detections(fps, id, boxes, probs, l.side*l.side*l.n, classes, w, h);
            free(id);
            free_image(val[t]);
            free_image(val_resized[t]);
        }
...
void validate_yolo4_recall(char *cfgfile, char *weightfile)
{
    network net = *parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    for(i = 0; i < m; ++i){
        ...
        if (nms) do_nms_y4(boxes, probs, side*side*l.n, 1, nms);
...
void test_yolo4(char *cfgfile, char *weightfile, char *filename, float thresh)
{
    image **alphabet = load_alphabet();
    network net = *parse_network_cfg(cfgfile);
    if(weightfile){
    ...
    while(1){
        ...
        draw_detections_y4(im, l.side*l.side*l.n, thresh, boxes, probs, voc_names, alphabet, 20);
        save_image(im, "predictions");
        show_image(im, "predictions", 0);
        free_image(im);
        free_image(sized);
...
void run_yolo4(int argc, char **argv)
{
    ...
    char *weights = (argc > 4) ? argv[4] : 0;
    char *filename = (argc > 5) ? argv[5]: 0;
    if(0==strcmp(argv[2], "test")) test_yolo4(cfg, weights, filename, thresh);
    else if(0==strcmp(argv[2], "train")) train_yolo4(cfg, weights);
    else if(0==strcmp(argv[2], "valid")) validate_yolo4(cfg, weights);
    else if(0==strcmp(argv[2], "recall")) validate_yolo4_recall(cfg, weights);
    else if(0==strcmp(argv[2], "demo")) demo(cfg, weights, thresh, cam_index, filename, voc_names, 20, frame_skip, prefix, avg, .5, 0,0,0,1);
}
